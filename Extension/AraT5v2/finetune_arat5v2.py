...
def preprocess_function(examples, tokenizer, max_seq_length=1024):
    ...
    # Tokenize the inputs with consistent padding and truncation
...
def train_and_finetune(train_file, dev_file, output_dir, epochs=5, learning_rate=1e-4):
    ...
    # Model & Tokenizer (using the AraT5v2 model)
    model_name = "UBC-NLP/AraT5v2-base-1024"
    tokenizer = AutoTokenizer.from_pretrained(model_name, use_fast=False)
    model = AutoModelForSeq2SeqLM.from_pretrained(model_name)

    # Load datasets from local JSON files (each line is a JSON object)
...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune AraT5v2 model for text correction."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir", type=str, default="araT5v2_gec_checkpoints",
